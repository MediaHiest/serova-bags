...
export default function AdminBrandsPage() {
  ...
  function startEdit(brand: Brand) {
    ...
    setForm({ name: brand.name, slug: brand.slug, description: "", isActive: brand.isActive });
    setShowForm(true);
  }
  ...
  return (
    <div>
      <div className="admin-page-header">
        <h1 className="text-xl sm:text-2xl font-medium text-text-dark">Brands</h1>
        <button
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="admin-card max-w-md space-y-3 mb-8">
          ...
          <div className="flex gap-3">
            ...
            <button type="button" className="text-sm text-text-muted" onClick={() => setShowForm(false)}>
              Cancel
            </button>
          </div>
        </form>
          ...
          <tbody>
            {brands.map((brand) => (
              <tr key={brand.id} className="border-b border-gray-50">
                ...
                <td className="py-3 pr-4 text-text-muted">{brand.slug}</td>
                <td className="py-3 pr-4">{brand._count.products}</td>
                <td className="py-3 pr-4">{brand.isActive ? "Active" : "Inactive"}</td>
                <td className="py-3 flex gap-2">
                  <button className="text-xs underline" onClick={() => startEdit(brand)}>
                    Edit
                  </button>
                  <button className="text-xs text-red-600 underline" onClick={() => deleteBrand(brand.id)}>
                    Delete
                  </button>
                </td>
              </tr>
